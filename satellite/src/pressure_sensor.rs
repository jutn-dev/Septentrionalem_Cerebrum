...
//TODO add hardware software reset thing
    ...
    pub fn read_temp(&mut self) -> Result<f32, EspError> {
        let baro_address = 0x5C;
        let mut temp_l = [0; 1];
        let mut temp_h = [0; 1];
        self.i2c_driver
            .write_read(baro_address, &[0x2C], &mut temp_l, BLOCK)?;
        self.i2c_driver
            .write_read(baro_address, &[0x2B], &mut temp_h, BLOCK)?;

        let temp = ((temp_l[0] as u16) << 8 | temp_h[0] as u16) as f32 / 100.;
        Ok(temp)
    }

    pub fn read_pressure(&mut self) -> Result<f32, EspError> {
        let baro_address = 0x5C;
        let mut pressure_h = [0; 1];
        let mut pressure_l = [0; 1];
        let mut pressure_xl = [0; 1];
        self.i2c_driver
            .write_read(baro_address, &[0x2A], &mut pressure_h, BLOCK)?;
        self.i2c_driver
            .write_read(baro_address, &[0x29], &mut pressure_l, BLOCK)?;
        self.i2c_driver
            .write_read(baro_address, &[0x28], &mut pressure_xl, BLOCK)?;
