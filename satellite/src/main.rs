...
fn main() -> Result<(), EspError> {
    ...
    std::thread::Builder::new()
        .stack_size(7000)
        .spawn(move || pressure_sensor_thread(i2c_driver_clone, tx_clone).unwrap())
        .unwrap();
    ...
    std::thread::Builder::new()
        .stack_size(7000)
        .spawn(move || {
            gyro_sensor_thread(i2c_driver_clone, tx_clone).unwrap();
        })
        .unwrap();
*/
    let i2c_driver_clone = Arc::clone(&i2c_driver);
    ...
    let (time_tx, time_rx) = std::sync::mpsc::channel::<u64>();
    std::thread::Builder::new()
        .stack_size(7000)
        .spawn(move || {
            gps_thread(i2c_driver_clone, tx_clone, time_tx).unwrap();
        })
        .unwrap();
    ...
    let timer_driver = LedcTimerDriver::new(
        ...
        &TimerConfig::new().frequency(Hertz(50)),
    )?;
...
fn gyro_sensor_thread(
    i2c: Arc<Mutex<I2cDriver>>,
    tx: Sender<DataTypes>,
) -> Result<(), EspError> {
    let mut i2c_driver = i2c.lock().unwrap();
    let pressure_sesnor = GyroSensor::init(&mut i2c_driver)?;
    drop(i2c_driver);
    loop {
        FreeRtos::delay_ms(200);
        let mut i2c_driver = i2c.lock().unwrap();

        let gyro = pressure_sesnor.read_gyro(&mut i2c_driver)?;
...
fn gps_thread(i2c: Arc<Mutex<I2cDriver>>, tx: Sender<DataTypes>, mut time_tx: Sender<u64>) -> Result<(), EspError> {
    let mut i2c_driver = i2c.lock().unwrap();
    ...
    loop {
        FreeRtos::delay_ms(100);
        let Some(data_vec) = gps_driver.get_data(&i2c, &mut time_tx)? else {
            continue;
        };
